...
*source/demo directory 내부의 모든 파일을 설치하셔야 동작 가능합니다.

# IAP: 얼굴 및 손-물체 인식을 활용한 지능형 학습 감시 시스템

## Introduction
COVID-19 이후 비대면 교육과 자율 학습 환경이 일상화되면서 많은 학습자들이 집중력 저하를 겪고 있습니다. 특히 지도자의 직접적인 피드백이 어려운 온라인 환경에서는 산만함, 졸음, 스마트폰 사용 등 외부 요인에 쉽게 노출되며, 이는 학습 효과 저하로 이어지는 문제가 있습니다. 하지만 기존 시스템들은 주로 수동으로 캠 화면을 확인하거나 얼굴 인식 기반의 단순 시선 추적에 의존하여, 실질적인 집중도 확인을 정량적으로 수행하기 어렵다는 한계를 가지고 있습니다.

본 프로젝트는 이러한 한계를 극복하고자, 실시간 집중도 판단이 가능한 지능형 학습 감시 시스템을 제안합니다. 이 시스템은 웹캠 영상만으로 얼굴, 손, 그리고 학습 중인 물체를 탐지하며, 얼굴 자세와 손에 쥔 물체 정보를 종합 분석하여 학습자의 집중 여부를 판단합니다. 특히, NVIDIA Jetson Nano 보드를 플랫폼으로 활용하여 엣지 환경에서도 원활히 동작하도록 모델 경량화와 실시간 처리 최적화 기법을 적용하였습니다.

더불어, 본 시스템은 사용자 맞춤형 기준 자세(calibration)를 설정해 각 개인의 고유한 자세 특성을 반영합니다. Head Pose 데이터를 활용하여 초기 기준 자세를 정의하고, 이를 바탕으로 YOLO 탐지 결과와 Head Pose 정보를 통합 분석합니다. 산만함, 졸음, 스마트폰 사용, 집중 상태 등의 행동을 rule-based 및 슬라이딩 윈도우 기법으로 정밀하게 판단함으로써, 순간적인 변화에 흔들리지 않고 안정적이며 신뢰성 높은 집중도 평가가 가능합니다.

이처럼 개인별 맞춤 기준과 다중 데이터 융합 분석을 통해, 비대면 학습 환경에서도 학습자의 주의력과 집중도를 정량적이고 실시간으로 모니터링할 수 있는 혁신적인 솔루션을 제공합니다.
